/**
 * Future Self Engine — pure, deterministic, no I/O.
 * Projects next discipline level, streak projection, and future-identity messages for widgets.
 */
...
export function generateFutureSelf(input: FutureSelfInput): FutureSelfResult {
  ...
  // 2. Proche du prochain niveau (1–5 jours restants)
  ...
  // 3. Identity future (streak >= 14) — variante déterministe
  if (currentStreak >= 14) {
  ...
  // 4. Loin du prochain niveau (daysRemaining > 5, streak 3–13)
  ...
  // 5. Projection de streak (streak >= 3, proche du niveau déjà géré en 2)
  if (currentStreak >= 3) {
...
/** Default futureSelf for empty state (no user or no habits). */
export function getDefaultFutureSelf(): FutureSelfResult {
  return {
    nextLevel: { name: 'Builder', daysRemaining: 3 },
