...
import { DOMAIN_META, QUALITY_LABELS } from '../data/karmicSeedCatalog';
...
function formatDraft(draft: Partial<MorningDraft>): string {
    const lines: string[] = [];
    ...
    if (draft.domain) lines.push(`Domaine choisi: ${DOMAIN_META[draft.domain]?.label ?? draft.domain}`);
...
function formatProgress(
    ...
    if (plotProgress) {
        const weak = Object.entries(plotProgress)
            .sort(([, a], [, b]) => a - b)
            .slice(0, 2)
            .map(([d, n]) => `${DOMAIN_META[d as SeedDomain]?.label ?? d}: ${n} graines`);
        if (weak.length) parts.push(`Parcelles les plus faibles: ${weak.join(', ')}`);
    }
    if (qualities) {
        const low = (Object.entries(qualities) as [keyof InnerQualities, number][])
            .sort(([, a], [, b]) => a - b)
            .slice(0, 2)
            .map(([k, v]) => `${QUALITY_LABELS[k]}: ${v}%`);
...
export function buildCacheKey(ctx: KarmicCoachRequestContext): string {
    const { step, draft, qualities, plotProgress } = ctx;
    return JSON.stringify({ step, draft, qualities, plotProgress });
}
